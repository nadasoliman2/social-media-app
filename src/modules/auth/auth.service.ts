...
this.notification= notificationService
 ...
 private  SendEmailOtp = async ({
  ...
  const isBlocked: number | null = await  this.redis.ttl(blockKeyValue);
...
if(!match){
    throw new  NotFoundException(" Invalid login credentials")}
if(FCM){
  await this.redis.addFCM(Existcheckuser._id.toString(), FCM)
  const tokens = await this.redis.getFCMs(Existcheckuser._id.toString())
  if(tokens?.length){
  await this.notification.sendNotifications({tokens: tokens, data:{title:"New Login Detected", body:`new login ${new Date().toLocaleString()}`}})
}}
